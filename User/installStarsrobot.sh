...
#5、如果编译通过后请执行starshost_network.sh这个脚本，把Starsrobot的网络环境与环境变量添加
#6、请把58-stars.rules这个规则文件拷贝到 /etc/udev/rules.d/目录下面， 可以执行 sudo cp 58-stars.rules /etc/udev/rules.d/
